...
public extension View {
    ...
    func bottomSheet<Content: View>(
        ...
        onDetentChange: ((BottomSheet.Detent) -> Void)? = nil,
...
public struct BottomSheet {
    ...
    private static var sheetDelegate: BottomSheetDelegate?
    ...
    fileprivate static func present<Content: View>(
        detents: [Detent],
        onDetentChange: ((Detent) -> Void)? = nil,
...
//        hostingController.view.layer.opacity = Float(backgroundOpacity ?? 1.0)
        ...
        if let sheet = nav.sheetPresentationController {
            ...
            sheetDelegate = BottomSheetDelegate { selectedIdentifier in
                var detent: BottomSheet.Detent = .fixed(0)

                if #available(iOS 16.0, *) {
                    guard let selectedIdentifier else { return }

                    if selectedIdentifier == UISheetPresentationController.Detent.Identifier.medium {
                        detent = .medium
                    } else if selectedIdentifier == UISheetPresentationController.Detent.Identifier.large {
                        detent = .large
                    } else if let match = detents.first(where: { $0.asUIKitDetent?.identifier == selectedIdentifier }) {
                        detent = match
                    }
                } else {
...
@available(iOS 15, *)
final class BottomSheetDelegate: NSObject, UISheetPresentationControllerDelegate {
    let onDetentChange: (UISheetPresentationController.Detent.Identifier?) -> Void

    init(onDetentChange: @escaping (UISheetPresentationController.Detent.Identifier?) -> Void) {
